...
- *С захватывающей группой*. Если в регулярном выражении указать захватывающую группу (например, `^@(\S+)-.*\.md$`), то оно будет работать аналогичной первой опции, но с большей гибкостью, позволяя создавать любые форматы имен файлов (например, `@citekey-название.md`).
