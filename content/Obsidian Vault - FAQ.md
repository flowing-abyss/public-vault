...
url:
  - "[flowing-abyss](https://flowing-abyss.com/Obsidian-Vault----FAQ)"
icon: ✏️
color: "#b87535"
created: 2025-12-13T19:22:49+07:00
updated: 2026-09-13T14:45:54+07:00
share: true
...
- **Что делать с моим старым хранилищем?**
	...
	- Переносить всё скопом не нужно
		- вы переходите не просто из папки в папку, а меняете сам метод мышления.
- **Что если какие-то плагины умрут (например, Dataview или Templater)?**
	...
	- Касательно экосистемы
		- если перестанут работать гиганты вроде Dataview/Templater, то сообщество либо починит их пулл-реквестами/форками, либо быстро найдет альтернативы.
...
- ~~[Inter](https://fonts.google.com/specimen/Inter)~~ → [IBM Plex Sans](https://fonts.google.com/specimen/IBM+Plex+Sans) для интерфейса
- ~~[Rubik](https://fonts.google.com/specimen/Rubik)~~ → [IBM Plex Sans](https://fonts.google.com/specimen/IBM+Plex+Sans) для текста
- ~~[Tinos](https://fonts.google.com/specimen/Tinos)~~ → [IBM Plex Serif](https://fonts.google.com/specimen/IBM+Plex+Serif) для заголовков
- ~~[Roboto Mono](https://fonts.google.com/specimen/Roboto+Mono)~~ → [IBM Plex Mono](https://fonts.google.com/specimen/IBM+Plex+Mono) для кода
...
![[Obsidian Vault - FAQ 20260419110212487.png]]
...
- `/today` – для старта и/или конца дня
	...
	- Я бы рекомендовал этот брифинг не только читать как факт, но и сразу же комментировать - соглашаться с выводами, не соглашаться, поддерживать и развивать
- `/week` – итоги недели
- `/month` – итоги месяца
...
- `/note` – атомные заметки в base/notes/
	- `/flashcard` – карточки для Spaced Repetition и Anki
		- Перед созданием показывает пример карточки и ждёт подтверждения
- `/project` – проектные заметки в projects/
- `/source` – источники (книги, статьи, видео и прочее)
- `/people` – контакты, авторы, организации
- `/addition` – дополнения к существующим заметкам (конспекты, встречи, отчёты и прочее)
	- Двухшаговый процесс
...
- `/structure` – структура хранилища, куда что класть
	...
	- `/system-note` – структурные заметки (мета-заметки, проблемы, иерархии)
...
- `/discourse` – [[#Discourse Graph|научный фреймворк]] для структурированных исследований
...
- `/task-inline` – [GTD-задачи](https://habr.com/en/articles/833654/)
- `/task-note` – проектные задачи в стиле [Linear](https://linear.app/)

==Редактирование:==<br>
Скиллы для редактирования и форматирования текста внутри заметок.
- `/editor` – редактирование текста с помощью ИИ через аннотации
...
- `/proofread` – бережная вычитка орфографии, пунктуации и грамматики без изменения авторского стиля
- `/humanizer` – убирает характерные ИИ-формулировки, сохраняя смысл и авторский голос
	- В отличие от `/proofread`, работает не с ошибками, а с манерой изложения; подходит и для готового текста, и для ответов самого агента
- `/verify` – дотошная проверка утверждений, отмеченных `[?]`
- `/markdown` (Kepano) – напоминает агенту синтаксис Obsidian

==Утилитные:==<br>
Инструменты для обслуживания и расширения самой системы.
- `/bases` (Kepano) – динамические таблички
...
- `/manifest` – чтение и проверка схем метаданных
	- Находит манифест, который применяется к заметке, и учитывает всю цепочку наследования
	- Объясняет доступные свойства, их типы и ограничения
	- Помогает найти ошибки Metadata Validator или спроектировать манифест для нового типа заметок
- `/vault-check` – аудит всего хранилища
	- Находит невалидные метаданные, битые ссылки, заметки не в своей папке, а также шаблоны, которые разошлись с манифестами
	- По умолчанию только читает. Сначала покажет сводку по причинам, а исправлять начнёт только после того, как вы скажете что именно
	- Достаточно написать "проверь хранилище" или "найди битые ссылки"
- `/cli` (Kepano) – управление Obsidian из [терминала](https://help.obsidian.md/cli)
