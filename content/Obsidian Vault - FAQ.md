...
updated: 2026-02-28T20:04:00+07:00
share: true
...
Скиллы задают для каждого структурного элемента правильную директорию, frontmatter и метаданные. Это позволяет агенту принимать корректные решения при создании заметок, не нарушая архитектуру хранилища.
...
- `/text-editor` – редактирование текста с помощью ИИ через аннотации
	- Оставляете в тексте директивы и выделения, которые определят точечные изменения
		- `%%директивы%%` – выполняются буквально
		- `/==выделения==` – слабые или расплывчатые места, которые агент переписывает самостоятельно по контексту
