...
updated: 2026-05-04T22:29:36+07:00
share: true
...
> <br>• • •<br>
> Важно отметить, что я достаточно точно описал все инструменты, как рекомендует Anthropic.[^9] Я даже написал отдельный скилл для тестирования через OpenCode с разными моделями. Это всё к тому, что агент должен приоритетно использовать гибридный поиск без явного указания.
...
[^6]: `Hsu, Hsin-Ling, and Jengnan Tzeng. “DAT: Dynamic Alpha Tuning for Hybrid Retrieval in Retrieval-Augmented Generation.” arXiv:2503.23013. Preprint, arXiv, March 29, 2025.` [DOI](https://doi.org/10.48550/arXiv.2503.23013) | Хотя есть другая статья, в которой по сути говорится какой бы коэффициент подошел в Convex Combination для PKM-системы. Всё же я не уверен, что без внутренних тестов стоило бы так слепо переходить на этот алгоритм как на дефолтный. Поэтому сначала я скорее всего сделаю отдельную опцию для выбора алгоритма.
[^7]: Пожалуй, самый крутой вариант для мультиязычного хранилища [qwen3-embedding-8b](https://openrouter.ai/qwen/qwen3-embedding-8b) (избыточно огромный контекст + в топе по [MTEB](https://huggingface.co/spaces/mteb/leaderboard)). Немного попроще, но которую можно [[Comparison of local embedding models|запустить локально]] – [bge-m3](https://openrouter.ai/baai/bge-m3) (тоже мультиязычная и крайне неплохая по метрикам).
